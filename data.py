...
from utils import ramp_value, schedule_scale
    ...
    def __getitem__(self, index: int) -> dict[str, torch.Tensor | str]:
        ...
        if self.train:
            augmentation_scale = schedule_scale(
                self.get_epoch(),
                self.config.scheduled_augmentation_start_epoch,
                self.config.scheduled_augmentation_ramp_epochs,
                self.config.scheduled_augmentation_ramp_schedule,
            )
            augmentation_config = replace(
                self.config,
                random_crop=0.0,
                color_jitter=ramp_value(self.config.color_jitter_start, self.config.color_jitter, augmentation_scale),
                gaussian_blur=ramp_value(self.config.gaussian_blur_start, self.config.gaussian_blur, augmentation_scale),
                random_grayscale=ramp_value(self.config.random_grayscale_start, self.config.random_grayscale, augmentation_scale),
            )
